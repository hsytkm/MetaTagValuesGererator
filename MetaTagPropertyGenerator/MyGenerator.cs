...
{
    [Generator]
    public sealed class MetaTagPropertyGenerator : ISourceGenerator
    {
        internal const string AttributeName = nameof(MetaTagPropertyGenerator) + "Attribute";
        ...
        private sealed class SyntaxReceiver : ISyntaxReceiver
        {
            ...
            public void OnVisitSyntaxNode(SyntaxNode syntaxNode)
            {
                ...
                var attr = fields.SelectMany(x => x.AttributeLists)?.SelectMany(x => x.Attributes)
                    .FirstOrDefault(x => x.Name.ToString() is nameof(MetaTagPropertyGenerator) or AttributeName);
